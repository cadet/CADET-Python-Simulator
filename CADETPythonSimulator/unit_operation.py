...
class DeadEndFiltration(UnitOperationBase):
    ...
    def compute_residual(
        ...
        if cake_vol_dot > 1e-16:
            self.residuals['cake']['c_in'] =\
                c_cake_in - n_cake_dot/cake_vol_dot
        else:
            self.residuals['cake']['c_in'][:] = 0.0
        ...
        if permeate_vol_dot > 1e-16:
            ...
            self.residuals['permeate_tank']['c_in'][:] = 0.0
    ...
    def initialize_initial_values(self, t_zero: float):
        ...
        if cake_vol_dot > 1e-16:
            self.states['cake']['c_in'] = n_cake_dot/cake_vol_dot
        else:
            self.states['cake']['c_in'][:] = 0.0
